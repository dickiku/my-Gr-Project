        ...
        <div class="row">
            ...
            <?= $this->session->flashdata('pesan'); ?>
                <div class="card mb-3 mx-auto">
                    ...
                    <hr>
                    <div class="card-body">
                        <form method="post" action="<?= base_url('dpc/tambah_proses') ?>" enctype="multipart/form-data">
                            <?php if($user['id_kab']) { ?>
                                <div class="form-group">
                                    <label>DPC Kab/Kota</label>
                                    <!-- <select name="dpc_kab_kota" class="form-control">
                                            <option value="">--Pilih Wilayah--</option>
                                            <?php foreach($wilayahKabById as $row) : ?>
                                            <option value="<?= $row->id_kab ?>"><?= $row->nm_kab ?></option>
                                            <?php endforeach; ?>
                                    </select> -->
                                    <?php foreach($wilayahKabById as $row) : ?>
                                        <input type="text" name="dpc_kab_kota" value="<?= $row->id_kab ?>" class="form-control" readonly>
                                    <?php endforeach; ?>
                            ...
                            <?php } else { ?>
                                <div class="form-group">
                                    <label>DPC Kab/Kota</label>
                                    <select name="dpc_kab_kota" class="form-control" readonly>
                                        <!-- <option value="">--Pilih Wilayah--</option> -->
                                        <?php foreach($wilayahKabById as $row) : ?>
                                            <option value="<?= $row->id_kab ?>"><?= $row->nm_kab ?></option>
                                        <?php endforeach; ?>
