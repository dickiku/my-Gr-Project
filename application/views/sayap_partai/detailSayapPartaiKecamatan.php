...
<?php foreach($sayap as $row) : ?>
    <?php $cekKecamatan = $this->Sayap_partai_model->getDataSayapPartaiDomKec($wk['id_kec'],$row['id_sayap_partai']); ?>
<?php endforeach; ?>
<?php endforeach; ?>
<?php if($cekKecamatan->num_rows() > 0) { ?>
    
<?php } else { ?>
    <div class="container ml-2">
        <a href="<?= base_url('sayap_partai/tambahDomSayapPartaiKec') ?>" class="btn btn-primary mb-3">+ Tambah Data</a>
    </div>
<?php } ?>
            ...
            <?= $this->session->flashdata('pesan') ?>
                <div class="card mb-3 mx-auto">
                    <div class="card-body">
                        ...
                        <a href="<?= base_url('sayap_partai/editDomSayapPartaiKec/').$row['id_dom_sayap_partai'] ?>" class="btn btn-lg btn-warning mb-2">Edit</a>
                        <table class="table table-bordered table-striped table-hover" id="myTable">
                            <tbody>
                                <tr>
                                    <td><strong>Wilayah</strong></td>
                                    <td>Sayap Partai Kecamatan</td>
                                </tr>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Susunan Kepengurusan Kecamatan</h3>
                </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <?php if($user['level'] == 'Super Admin') : ?>
                                    <a href="<?= base_url('sayap_partai/editKepengurusanKecamatan/').$kp['id_pengurus_sayap_partai'] ?>" class="btn btn-warning btn-md">
                                        Edit
                                    </a>
                                    <a href="<?= base_url('sayap_partai/hapusKepengurusanKecamatan/').$kp['id_pengurus_sayap_partai'] ?>" class="btn btn-danger btn-md" onclick="return confirm('Apakah yakin anda ingin menghapus data ini?')">
                                        Hapus
                                    </a>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="<?= base_url('sayap_partai/tambah_kepengurusan_proses_kecamatan') ?>" method="post">
            ...
            <div class="form-group">
                ...
                <?php foreach($wilayahKec as $wk) : ?>
                <input type="hidden" name="id_kec" value="<?= $wk['id_kec']?>" class="form-control">
                <input type="text" value="<?= $wk['nm_kec']?>" class="form-control" disabled>
                <?php endforeach; ?>
            </div>
            <div class="form-group">
