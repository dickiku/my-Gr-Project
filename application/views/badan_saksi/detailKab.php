...
<?php foreach($kabupaten as $kab) : ?>
    <?php $cek = $this->Badan_saksi_model->getDataBsSk(2,$kab['id_kab']); ?>
<?php endforeach ?>
...
<?php } else { ?>
    <div class="container ml-2">
        <?php foreach($kabupaten as $kab) : ?>
            <a href="<?= base_url('badan_saksi/tambahDataSk/'.'2'.'/'.$kab['id_kab']) ?>" class="btn btn-primary mb-3">+ Tambah Data</a>
        <?php endforeach ?>
    </div>
<?php } ?>
                ...
                <div class="card mb-3 mx-auto">
                    <div class="card-body">
                        <?php foreach($keterangan as $ket) : ?>
                            <a href="<?= base_url('badan_saksi/showEditSk/'.$ket['id_badan_saksi_sk'].'/'.'2') ?>" class="btn btn-lg btn-warning mb-2">Edit</a>
                        <?php endforeach; ?>
...
<div class="container ml-2">
    <a href="" class="btn btn-primary mb-3" data-toggle="modal" data-target="#tambahPengurusBSKab">+ Tambah Data Kepengurusan Badan Saksi</a>
</div>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <?php foreach($kabupaten as $kab) : ?>
                    <h3 class="card-title">Susunan Kepengurusan Badan Saksi Kab <?= $kab['nm_kab']?></h3>
                    <?php endforeach; ?>
                </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <?php if($user['level'] == 'Super Admin') : ?>
                                    <a href="<?= base_url('badan_saksi/showEdit/'.$kp['id_badan_saksi_kab'].'/'.'3'.'/'.$kp['id_kab']) ?>" class="btn btn-warning btn-md">
                                        Edit
                                    </a>
                                    <a href="<?= base_url('badan_saksi/hapus_pengurus_kab/'.$kp['id_badan_saksi_kab']. '/' .$kp['id_kab']) ?>" class="btn btn-danger btn-md" onclick="return confirm('Apakah yakin anda ingin menghapus data ini?')">
                                        Hapus
                                    </a>
...
<!-- Modal Tambah-->
<div class="modal fade" id="tambahPengurusBSKab" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="<?= base_url('badan_saksi/tambah_pengurus_kab') ?>" method="post">
            ...
            <div class="form-group">
                ...
                <?php foreach($kabupaten as $kab) : ?>
                <input type="hidden" name="id_kab" value="<?= $kab['id_kab']?>" required="required">
                <select class="form-control" disabled>
                    <option value="<?= $kab['id_kab']?>"><?= $kab['nm_kab']?></option>
                </select>
                <?php endforeach; ?>
            </div>
            <div class="form-group">
