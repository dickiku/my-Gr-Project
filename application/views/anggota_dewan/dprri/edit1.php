            ...
            <div class="col-md-8">
                <div class="card mb-3 mx-auto">
                    <div class="card-body">
                        ...
                        <form method="post" action="<?= base_url('anggota_dewan/prosesEditDPRDRi/') ?>" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <label>Nama</label>
                                <input type="hidden" name="nama" class="form-control" value="<?= $row['nama_dewan'] ?>" required="required">
                                <input type="text" name="nama" class="form-control" value="<?= $row['nama_dewan'] ?>" required="required" disabled>
                            </div>
                            <div class="form-group">
                                <label>No HP</label>
                                <input type="text" name="no_hp" class="form-control" value="<?= $row['no_hp_dewan'] ?>" required="required">
                            </div>
                            <div class="form-group">
                                <label>Dapil</label>
                                <select class="form-control" name="dapil" required>
                                    ...
                                    <?php foreach($dapil as $dp) : ?>
                                        <?php if($dp['id_dapil_ri'] == $row['id_dapil']) : ?>
                                            <option value="<?= $row['id_dapil'] ?>" selected><?= $row['nama_dapil_ri']?></option>
                                        <?php else : ?>
                                            <option value="<?= $dp['id_dapil_ri'] ?>"><?= $dp['nama_dapil_ri']?></option>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                </select>
                            </div>
