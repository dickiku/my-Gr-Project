...
class Berita_posting_model extends CI_Model {

   public $perPage = 5;

   public function getChoice()
   {
      $this->db->from('tb_berita_posting');
      $this->db->join('tb_berita_category', 'tb_berita_category.id = tb_berita_posting. id_category');
      $this->db->where('choice', 'Y');
      $this->db->where('tb_berita_posting.is_active', 'Y');
      $this->db->order_by('tb_berita_posting.id', 'desc');
      $this->db->limit(4);
   ...
   public function getVideoGames()
   {
      $this->db->from('tb_berita_posting');
      $this->db->join('tb_berita_category', 'tb_berita_category.id = tb_berita_posting. id_category');
      $this->db->where('tb_berita_category.slug', 'video-game');
      $this->db->where('tb_berita_posting.is_active', 'Y');
      $this->db->order_by('tb_berita_posting.id', 'desc');
      $this->db->limit(5);
      return $this->db->get()->result();
   ...
   public function getPostingById($id)
   {
      $this->db->from('tb_berita_posting');
      $this->db->join('tb_berita_category', 'tb_berita_category.id = tb_berita_posting. id_category');
      $this->db->where('tb_berita_posting.id', $id);
      return $this->db->get()->row();
   }
   ...
   public function getPostingByCategory($category, $page)
   {
      ...
      $this->db->join('tb_berita_category', 'tb_berita_category.id = tb_berita_posting. id_category');
      $this->db->where('tb_berita_posting.is_active', 'Y');
      $this->db->where('tb_berita_category.slug', $category);
      $this->paginate($page);
      $this->db->order_by('tb_berita_posting.id', 'desc');
      return $this->db->get()->result();
   }
   ...
   public function countPosting($category = null)
   {
      ...
      $this->db->where('tb_berita_posting.is_active', 'Y');  
      return $this->db->count_all_results('tb_berita_posting');
   }

   public function _getIdCategory($category)
   {
      $this->db->where('slug', $category);
      return $this->db->get('tb_berita_category')->row();
   }
